...
*Product Requirements Document — what we're building, for whom, and why. For how it's built, see [`TDD.md`](./TDD.md). For the authoritative current database schema, see [`DATA_MODEL.md`](./DATA_MODEL.md). For step-by-step user flows, see [`USERFLOW.md`](./USERFLOW.md). For screen inventory, brand tone, and design constraints, see [`DESIGN_BRIEF.md`](./DESIGN_BRIEF.md). For build status, milestones, and what's next, see [`ENG_PLAN.md`](./ENG_PLAN.md).*
...
- **Evening session** (at a user-picked time): the second and final exercise block. Content leans strength/recovery depending on what the regime calls for. Originally spec'd as sunset-anchored (geolocation-based); replaced with a user-picked time — see `ENG_PLAN.md`'s build history for why.
...
- **Escalation + pause** on pain spikes / "made it worse" → rollback + "stop & consult a professional" messaging above a threshold. Runs in real time on every Session Log write, not gated to the weekly adjustment cycle.
...
**Tool access, not MCP.** Exercise-library and current-regime lookups are implemented as **plain tool-use function calling**, not a standalone MCP server. MCP earns its complexity when multiple external clients/tools need to interoperate; for a single first-party app calling its own database from its own backend, function calling is materially less infrastructure to stand up and maintain for v1.
...
**Change ceilings (v1 defaults).** These are evidence-informed starting points drawn from sports-rehab load-management conventions (the "10% rule" for weekly training-load progression, and the 0–10 pain "traffic light" system used across PT clinics for hold/progress/regress decisions) — **not clinical prescriptions**. Per Pre-launch validation below, a PT/clinical advisor must review and sign off on these numbers before launch. (`ENG_PLAN.md`'s literature cross-check found the "10% rule" itself is weaker evidence than "evidence-informed" implies — see that doc before treating these numbers as settled.)
...
Every Flow A/B step that touches the LLM has a defined failure path — none of them ever result in an unhandled error reaching the user or an unsafe regime slipping through. Product-level requirements below; see `TDD.md` for the concrete implementation.

**Two separate validation layers, run in order:**

1. **Structural validation** — a schema check on the LLM's drafted regime object: required fields present, `exercise_list` non-empty, sets/reps/duration within sane numeric ranges, no duplicate exercise entries, valid `session_slot` enum values. Catches malformed output before it's even a candidate for clinical review.
2. **Clinical validation** — `validateRegime(draft, riskTier, previousRegime?)`, unchanged from above. Only runs on output that already passed structural validation.

**Invalid or nonexistent exercise ID.** Handled inside the tool-use loop itself: the exercise-library tool validates the requested ID against the database before returning a result. An invalid ID returns an error in the same turn, and the model is expected to self-correct, capped at a small number of correction attempts before falling through to the outer retry policy.

**Model/API down or rate-limited.**

- **Flow A (onboarding).** The generation job gains a `failed` state with a retry count. The job handler retries with exponential backoff. If all retries exhaust, the user is **not** blocked — they're handed the closest-matching general preset for their risk tier so onboarding still completes same-day, and the job is flagged for admin review.
- **Flow B (scheduled adjustment).** If the model/API is unreachable during a cron run, the safe default is to **hold** — skip the adjustment for that user this cycle. Hold is already a clinically valid state, so this introduces no new concept. Logged as an Adjustment Event so it's distinguishable later from an LLM-chosen hold. Retried automatically next cron cycle.
- **Escalation monitor.** Rules-based only; never calls the LLM. The app's real-time safety backstop is structurally unaffected by model-provider outages, unlike Flow A/B.

**`validateRegime` fails repeatedly.** One retry, with the specific rejection reason fed back to the LLM as context. If the second attempt also fails: Flow A falls back to the nearest general preset for that risk tier (flagged for admin review); Flow B falls back to hold.

Using the same two fallback destinations regardless of *why* generation failed keeps this to one failure-handling story instead of three, and keeps the admin panel's "flagged for review" queue meaningful.
...
- **Prompt hygiene.** Free text is passed into the regime-generation prompt as clearly delimited user-content, never concatenated into system-level instructions — the goal is that even an adversarial free-text input can only ever influence which exercises get proposed, never bypass `validateRegime`'s bounds.
- **Red-flag leakage check.** The structured red-flag screen is explicitly the *only* gate before a regime is generated, but nothing stops a user from answering "no" on a structured question and then disclosing the same red flag in free text — e.g., "36 weeks pregnant, sciatica's been brutal" typed into the lifestyle-context box. **Resolution:** a lightweight secondary pass over free-text fields (a cheap classifier call, since this is classification, not generation) runs specifically to catch red-flag disclosures the structured screen missed, before Flow A's LLM-draft step. A hit routes the same way a structured red flag does: exit to "see a doctor," no regime generated.
- **Consistency across call sites.** Open confirmation needed: is onboarding free text re-fed into Flow B prompts on every adjustment cycle, or captured once and never touched again? If reused, the length-bounding and red-flag-scan logic must run at every call site that reads it, not just at intake.

### Pre-launch validation

Create a held-out test set of realistic trajectories (plateauing, worsening, contradictory) and have a PT/clinical advisor review proposed adjustments pre-launch. Track "reversal rate" from day one. (The admin scenario simulator described in `TDD.md` is built specifically to produce this test set.)
...
*What's in v1 and what's intentionally deferred.*

**In Scope**

- Onboarding & goal capture: User signs up and selects their reason for using the app (injury recovery, strength, mobility, etc.); the initial regime is generated via the hybrid Flow A described above. v1 treats a user as having a single primary `target_movement` — see Data Model note.
...
- Licensed PT-authored/annotated exercise content — v1's library carries no clinical annotation (contraindications, rehab-specific progressions, isometric/regression staging). Implication for v1: since exercise selection itself carries no clinical safety annotation, the Clinical Risk Framing guardrails are the only safety layer — v1 exercise selection logic defaults conservative until PT-annotated content exists.
- Funded upgrade — licensed exercise media: once funded, swap from the free demo GIF source to AscendAPI Shop (one-time purchase, no subscription), chosen for same-ecosystem continuity with the demo tier.
- PT/clinical document upload — let users with existing PT documentation upload it so the AI can use it instead of (or alongside) the onboarding questionnaire. If built, v1 of this feature should make the AI's read of the document advisory (surfaced for user confirmation), not autonomous.
- True multi-goal support (multiple concurrent `target_movement`s with independent tracking). The schema allows for it; the v1 product does not implement it.
...
See [`DESIGN_BRIEF.md`](./DESIGN_BRIEF.md) for target-user framing, brand tone, screen inventory (built vs. proposed), and design constraints, and [`USERFLOW.md`](./USERFLOW.md) for the actual step-by-step flows through the app. Neither is a substitute for real Figma designs — flag to team: needs to happen before engineering can produce a real timeline estimate for the remaining UI/UX overhaul, not just before build starts.

*[link to Figma — still TBD]*

# Data Model

See [`DATA_MODEL.md`](./DATA_MODEL.md) for the authoritative, implementation-accurate schema (kept in sync with `packages/db/prisma/schema.prisma`). Product-level intent, preserved here:

- **User** — goal type, risk tier (set at onboarding, not currently reassessed — see Open Question below), condition flags, target movement(s). Schema stores an array of target movements for forward compatibility, but v1 enforces a single primary movement at the UI/API layer; true multi-movement tracking is deferred (see Technical Scope).
- **Exercise** (library item) — category, target muscle group(s), difficulty, contraindications (currently empty for all rows — no PT-annotated content yet), progression group, media, source.
- **Regime** (versioned) — the object the agent modifies over time: exercise list (with sets/reps/duration/frequency/session slot), status, end reason, parent-version link supporting history/rollback.
- **Workout Session** — one per scheduled morning/evening slot per day; tracks exercise completion (what streaks are computed from). Distinct from Session Log, which tracks self-reported stats.
- **Session Log** — one per day, the daily stat/pain check-in bundled with the morning session: pain score, mobility/strength indicator, a "this made it worse" flag feeding the adverse-event guardrail and the escalation monitor.
- **Adjustment Event** — audit trail for the agent: trigger type (`scheduled_adjustment` vs. `escalation_rollback`), trailing window used, rationale, and a `was_reversed` flag that's the reversal-rate guardrail metric as a first-class field.
...
1. **Initial-regime generation logic — resolved.** Hybrid: rules-based red-flag screening and risk tiering gate the LLM, the LLM drafts using tool-use function calling grounded in the exercise library and assigns morning/evening slots, and structural validation plus a rules-based change-ceiling validator check the output before the user sees it. See Clinical Risk Framing > Regime Generation Architecture, and `TDD.md` for the current implementation (which now also includes a skeleton-preset retrieval step ahead of the LLM draft — see that doc).
2. **Streak design — partially resolved.** What it rewards is settled: completing at least one of the day's two sessions maintains the streak. Still open: what happens to the streak once a user's stated goal is met — does it end, convert to a maintenance streak, or something else?
3. **Notification scheduling — resolved.** Fixed at two notifications/day (morning wake, evening user-picked time — originally spec'd as sunset-anchored, changed after build; see `ENG_PLAN.md`). Delivered as local on-device notifications, not server push.
4. **Exercise library sourcing — resolved, both phases.** (a) *Data*: Free Exercise DB (public domain) for v1 logic. (b) *Demo visuals*: AscendAPI free tier, disposable, demo-only. (c) *Funded launch*: AscendAPI Shop, Standard or Premium tier ($500–$600 one-time), chosen for same-ecosystem continuity with the demo tier.
5. **Elder accessibility — resolved.** Font scaling and larger tap targets shipped as the v1 baseline (see `ENG_PLAN.md`).
6. **Escalation thresholds — resolved.** Concrete per-tier trigger table defined above, running in real time on every Session Log write.
7. **Validator scope — resolved.** One shared function, `validateRegime(draftRegime, riskTier, previousRegime?)` — absolute bounds always checked, delta check only when `previousRegime` is supplied. Runs after a separate structural validation pass.
8. **LLM reliability & failure handling — resolved.** See Clinical Risk Framing > LLM Reliability & Failure Handling above and `TDD.md` for implementation.
9. **Free-text input handling — resolved.** See Clinical Risk Framing > Free-Text Input Handling above. Still open: whether free text is re-fed into Flow B prompts.
10. **`target_movement` cardinality — resolved for v1.** Schema keeps the array for forward compatibility; product enforces single-selection. True multi-goal tracking is deferred.
11. **Model choice for Flow A/B — resolved, revisited post-launch of beta.** Originally decided as Claude Sonnet 5 for both flows. As of the current build, production defaults to a cheaper Gemini model instead (cost-conscious swap), with Claude models still fully available via the admin experimentation dashboard for comparison or more complex tasks. See `TDD.md` for the current default and rationale, and `ENG_PLAN.md`'s Unit Economics item — this is exactly the kind of decision that item was meant to drive, now backed by real per-call cost data instead of an estimate.
...
- **Habit-loop structure:** a fixed, forgiving two-session daily rhythm with content-aware exercise placement and a streak that only needs one of the two sessions. This is a deliberate product bet, not incidental — it's the mechanism meant to solve the adherence problem named in Problem Statement.
...
**Voluntary cancellation.** Minimal v1 version, deliberately decoupled from the still-open free-tier question below: cancellation takes effect at the end of the current billing period (standard Stripe behavior, less punitive than immediate cutoff), and capture a reason code at cancel time (feeds the churn guardrail metric). On cancellation, personalized-regime access downgrades to presets-only.

**Open item:** exact price point and whether any free tier persists post-trial. This also gates the fuller win-back flow beyond the minimal cancellation handling above. See `ENG_PLAN.md` for build status — billing UI exists as a non-blocking preview, no real Stripe integration yet.
...
- **Guardrails (adverse event rate, churn, reversal rate):** adverse events and churn pull from Session Log `flag` and account-status events (including the voluntary-cancellation reason code); reversal rate pulls directly from Adjustment Event's `was_reversed` field, segmented by `trigger_type`.
...
- [ ] Model LLM inference cost per active user per week (Flow B cadence + real-time escalation monitor on every Session Log write) against the ~$14.99/mo price point, to confirm the "no clinician overhead" pricing thesis actually holds margin. The switch to a cheaper default model (see Open Question #11, `TDD.md`) is a first real move on this, but the full model hasn't been run.
...
- [ ] Roadmap/timeline — milestones, target launch date, and sequencing (e.g., when PT review happens relative to fundraising vs. launch). See `ENG_PLAN.md` for the closest current substitute.
- [ ] Ownership — assign owners to each open item/question (currently unassigned — solo build).
...
Architecture, tech stack, system design, and current implementation detail live in [`TDD.md`](./TDD.md) — that document is the technical answer to everything specified above, kept current against the real codebase rather than restated here.
