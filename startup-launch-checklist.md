...
| 16 | Webhooks with no signature check | A forged `payment_succeeded` POST can unlock premium features for free | Verify the signature against your webhook secret before processing the payload | TODO — now directly relevant, `/api/webhooks/clerk` exists (added for account deletion); confirm it verifies Clerk's webhook signature |
